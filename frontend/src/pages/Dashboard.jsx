...
const Dashboard = ({ user, progress = 0, onUserUpdate }) => {
  ...
  const fetchProgressData = async () => {
    try {
      ...
      const weeklyResponse = await fetch('https://aistudylab.onrender.com/api/progress/weekly', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      const summaryResponse = await fetch('https://aistudylab.onrender.com/api/progress/summary', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
